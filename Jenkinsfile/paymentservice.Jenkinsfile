...
// Ensure branch is Checkedout and take latest pull from the specified branch.
    stage("SCM Checkout") {
        dir("/var/www/microservices-demo/src/paymentservice") {
            sh "git fetch"
...
// Ensure that Docker image is build and push it into the repository.
    stage("Build") {
        dir("/var/www/microservices-demo/src/paymentservice") {
            sh "docker build -t gcr.io/google-samples/microservices-demo/paymentservice:v0.2.${env.BUILD_ID} ."
            sh "docker push gcr.io/google-samples/microservices-demo/paymentservice:v0.2.${env.BUILD_ID}"
        }
    }
...
// Ensure that it download the yaml file with the current jenkins build number and deploy it in its services.
    stage("Deploy"){
        dir("/var/www/microservices-demo/kubernetes-manifests/"){
            sh "curl -XGET --header 'PRIVATE-TOKEN: <private-token>' 'https://github.com/shahidshaikh1005/microservices-demo/blob/master/kubernetes-manifests/paymentservice.yaml' -o ./ad-service.yaml"
            sh "sed 's/__VERSION__/'${env.BUILD_ID}'/g' ./ad-service.yaml > ./paymentservice.yaml"
